...
**Cloudflare Turnstile:** Guest POST forms (`/login`, `/register`, `/forgot-password`) embed the managed widget (`templates/_turnstile.html`, site key `TURNSTILE_SITE_KEY`, `data-action="turnstile-spin-v2"`). Server-side `_turnstile_error()` → `_turnstile_siteverify()` POSTs to `https://challenges.cloudflare.com/turnstile/v0/siteverify` with `TURNSTILE_SECRET` (read live from env each request) and the token — **without** `remoteip` (Docker/cloudflared internal IPs can break siteverify). Siteverify forces IPv4 via a temporary `socket.getaddrinfo` override (timeout 3s) because Python/`urllib` otherwise often hangs for many seconds on broken Docker IPv6. Fail closed if secret/token missing or `success` is not true; flash messages distinguish missing secret, missing token, and invalid secret. Widget domains in the Cloudflare dashboard must include the public hostname (e.g. `srf.dronenerds.ch`) plus `localhost` for local testing. Host `.env` alone is not enough — the value must be visible inside the running container (`docker exec … printenv TURNSTILE_SECRET`), and `docker-compose.yml` must list `TURNSTILE_SECRET=${TURNSTILE_SECRET}` under `app.environment`.
