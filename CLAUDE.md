...
**Cloudflare Turnstile:** Guest POST forms (`/login`, `/register`, `/forgot-password`) embed the managed widget (`templates/_turnstile.html`, site key `TURNSTILE_SITE_KEY`, `data-action="turnstile-spin-v2"`). Server-side `_turnstile_error()` → `_turnstile_siteverify()` POSTs to siteverify with `TURNSTILE_SECRET` and token — **without** `remoteip`. Siteverify dials a **cached IPv4** (`_turnstile_resolve_ipv4`, 1h TTL) via `_HTTPSConnectionToIP` (SNI/Host = hostname; timeout 2.5s). At worker import, `_start_turnstile_dns()` **synchronously** warms the cache (prints `Turnstile DNS warmup ok` to stdout — visible in `docker logs`; measured ~4s Docker DNS on docker-public), then `_turnstile_dns_keeper` refreshes every 5 min. Fail closed if secret/token missing or `success` is not true. `docker-compose.yml` must pass `TURNSTILE_SECRET=${TURNSTILE_SECRET}`.
...
**Auth:** Session-based (`flask.session`). `login_required` decorator. Login accepts username or pilot email. On successful login `session.permanent = True` with `PERMANENT_SESSION_LIFETIME` of 30 days and `SESSION_REFRESH_EACH_REQUEST` so PWA users stay signed in across app restarts (cookie still HttpOnly/Secure/SameSite=Lax; logout clears the session). Admin `create_user` creates matching `profiles` row; `edit_user` / cascade `delete_user` in admin. Admin flag stored in `users.is_admin`. Two extra `users` columns (added via `CREATE TABLE` + idempotent `ALTER TABLE`): `email` and `is_approved` (default 1; self-registrations set 0).
