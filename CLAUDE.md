...
**When releasing** (user «Version X abschliessen», after release commit + push): move cards **Ready to implement** → **Implemented** only if actually implemented for that version. Cards not done stay in **Ready to implement**. Move the master card only when the full linked scope for that version is done. If spec and implementation diverge, alert the user before moving.
